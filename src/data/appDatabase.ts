...
export async function ensureAppDefaults() {
  await db.transaction('rw', db.settings, db.equipmentProfiles, async () => {
    if (!await db.settings.get('default')) await db.settings.put({ id: 'default', unit: 'kg', activeEquipmentProfileId: 'commercial-gym', trainingGoal: 'hypertrophy', preferredSplit: 'full_body', defaultDurationMinutes: 60 })
    const defaults = [
