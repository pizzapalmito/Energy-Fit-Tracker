...
- At the pinned commit, the combined dataset (`dist/exercises.json`) contains 876 exercise records; 873 of them reference start/end images (2 each, 1746 images total), matching the 873 image directories observed in the upstream tree. The `focused-strength-v2` output ships 465 selected exercises; exact included-media counts are recorded in the generated audit report. The remaining 3 upstream records have no bundled images.
